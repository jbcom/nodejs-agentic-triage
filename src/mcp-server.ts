...
server.tool(
  ...
  async ({ query }) => {
    try {
      ...
      return {
        content: [
          { type: 'text', text: `Error searching issues with query "${query}": ${error.message}` },
        ],
        isError: true,
      };
    }
...
server.tool(
  ...
  async ({ id, labels }) => {
    try {
      const issue = await octokit.getIssue(id);
      const remaining = issue.labels.filter(
        (l: any) => !labels.includes(typeof l === 'string' ? l : l.name)
      );
      ...
      return {
        content: [
          { type: 'text', text: `Error removing labels from issue #${id}: ${error.message}` },
        ],
        isError: true,
      };
    }
